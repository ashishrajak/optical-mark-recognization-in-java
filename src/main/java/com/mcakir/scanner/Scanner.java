...
//                putText(source.submat(roi), "(" + i + "_" + selection[2] + ")", new Point(rows.get(selection[2]).get(0, 0)), Core.FONT_HERSHEY_SIMPLEX, 0.3, new Scalar(0, 255, 0));
    ...
    public static StringBuffer  getAns(){
        StringBuffer string_ans=new StringBuffer();
        for(int i=0;i<answers.size();i++){
        
            if(answers.get(i)==null){
                string_ans.append(i+ " " +"null"+";"); 
            }
            if(answers.get(i)==(Integer)0){
                string_ans.append(i+ " " +"A"+";"); 
            }
            if(answers.get(i)==(Integer)1){
                string_ans.append(i+ " " +"B"+";"); 
            }
            if(answers.get(i)==(Integer)2){
                string_ans.append(i+ " " +"C"+";"); 
            }
            if(answers.get(i)==(Integer)3){
                string_ans.append(i+ " " +"D"+";"); 
            }
